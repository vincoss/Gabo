...
void UsartInterruptEventLogic(void);
...
/*
	...
	B	Powertrain control signals
	0 on/off
	1 run/brake
	2 cw/ccw

	Z	Signal activate commands (can be just copy those into final)
	Z=255
	Z?
*/
...
void ProcessCommand(char * command_in)
{
	switch (command_in[0])
	{
		case 'A': // Power on/off
		{
			if (command_in[1] == '?')
			{
				// Write response with current command value.
		...
		case 'B': // Left Powertrain speed
			...
			{
				// Write response with current command value.
				PrintValue('A', commandPowertrain);
			}
			else if (command_in[1] == '=') 
			{
				commandPowertrain = ParseCommand(command_in, 0);
			}
			break;
